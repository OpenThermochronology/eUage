...
// Hardwire RDAAM parameters
	...
	trapRmr0 = 0.83;   // FT rmr0 parameter for trap annealing typical often 0.81 to 0.82; HeFTy uses 0.83; Durango is 0.79
	psiUnits = PSI_CMSQ_TRACK;

// Alpha stopping distances in apatite (from Ketcham 2009)
	asDist.asU238 = 18.81;
	asDist.asU235 = 21.80;
 ...
 */
void RDAAM_InitFTAnnealingTraps(double ** &annealingTraps)
{
...
// In the first step, find age of the "oldest trap" persisting at present day.
  ...
  equivTime = 0;
    ...
    if (node == 0) break;
		if (fInit[node] < 0.999) {   // *** Change to 2/sum
...
// Now create fstate array, holding f of traps formed in each previous time step
	...
	int initEndpoint = (fInit[otNode] >= 0) ? otNode+1 : otNode;
...
// Now do the calculation again for all other time steps
	for (tsNode = numTTNodes-3; tsNode >= endNode; tsNode--) {
		equivTime = 0;
...
//	double lint0sq = 625.76*625.76;
	...
	endNode = 0;
	RDAAM_InitFTAnnealingTraps(annealingTraps);
	if (endNode > tTPath.size()*0.7) {  // Redo if not enough nodes are used (70% of path unused)
...
// It provides calues for variables heModelAge and heCorrModelAge, and overall
